...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	...
	// fmt.Printf("ingest snapshot from server %d, lastApplied update to %d.\n", i, cfg.lastApplied[i])
	return ""
}
...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			// fmt.Printf("server %d lastApplied = %d\n", i, cfg.lastApplied[i])
			cfg.mu.Unlock()
